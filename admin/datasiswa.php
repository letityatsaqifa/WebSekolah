  ...
  <body class="layout-fixed sidebar-expand-lg bg-body-tertiary">
          ...
          <div class="container-fluid">
            ...
            <div class="row">
              ...
              <div class="col-md-12">
                      ...
                      <tbody>
                        ...
                        ?>
                          <tr class="align-middle">
                            ...
                            <td class="op">
                                <button type="button" class="btn btn-warning btn-sm btn-edit-siswa" data-id="<?php echo $x['nisn']; ?>" data-bs-toggle="modal" data-bs-target="#editSiswaModal">
                                    <i class="bi bi-pencil-square"></i> Edit
                                </button>
                                
                                <button type="button" class="btn btn-danger btn-sm btn-hapus-siswa" data-id="<?php echo $x['nisn']; ?>">
                ...
                <!-- /.card -->
                <a href="tambahsiswa.php" class="add-button">Tambah Data</a>
              </div>
              <!-- /.col -->
            </div>
            <!--end::Row-->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form id="editSiswaForm">
                <div class="row mb-3">
                  <label for="edit_nisn" class="col-sm-2 col-form-label">NISN</label>
                  <div class="col-sm-10">
                ...
                <fieldset class="row mb-3">
                  ...
                  <div class="col-sm-10">
                    <div class="form-check">
                      <input class="form-check-input" type="radio" name="jeniskelamin" id="edit_jk_l" value="L" required>
                      <label class="form-check-label" for="edit_jk_l">Laki - Laki</label>
                    </div>
                    <div class="form-check">
                      <input class="form-check-input" type="radio" name="jeniskelamin" id="edit_jk_p" value="P" required>
                      <label class="form-check-label" for="edit_jk_p">Perempuan</label>
                    </div>
                  </div>
                </fieldset>
                <div class="row mb-3">
                  <label for="edit_kodejurusan" class="col-sm-2 col-form-label">Kode Jurusan</label>
                  <div class="col-sm-10">
                      <input type="text" class="form-control" id="edit_kodejurusan" name="kodejurusan" required placeholder="Masukkan kode jurusan (angka)">
                  </div>
                </div>
                <div class="row mb-3">
                  <label for="edit_agama" class="col-sm-2 col-form-label">Kode Agama</label>
                  <div class="col-sm-10">
                      <input type="text" class="form-control" id="edit_agama" name="agama" required placeholder="Masukkan kode agama (angka)">
                  </div>
                </div>
                <div class="row mb-3">
                ...
                <div class="row mb-3">
                  <label for="edit_alamat" class="col-sm-2 col-form-label">Alamat</label>
                  <div class="col-sm-10">
                      <textarea class="form-control" name="alamat" id="edit_alamat" required></textarea>
                  </div>
                </div>
                <div class="row mb-3">
                  <label for="edit_nohp" class="col-sm-2 col-form-label">No HP</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" id="edit_nohp" name="nohp" required>
                  </div>
                </div>
              </form>
            </div>
            <div class="modal-footer">
              ...
              <button type="button" class="btn btn-primary" id="saveSiswaButton">Simpan Perubahan</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <script>
      $(document).ready(function() {
          ...
          // FUNGSI EDIT SISWA
          $('#myTable tbody').on('click', '.btn-edit-siswa', function() {
              var nisn = $(this).data('id');
              $.ajax({
                  url: 'api_siswa.php',
                  type: 'GET',
                  data: { aksi: 'get_single', nisn: nisn },
                  dataType: 'json',
                  success: function(response) {
                      if (response.status === 'success') {
                          ...
                          $('#edit_nohp').val(siswa.nohp);
                          $('#edit_alamat').val(siswa.alamat);
                          $('#edit_kelas').val(siswa.kelas);
                          
                          // Mengisi input teks untuk Jurusan dan Agama
                          $('#edit_kodejurusan').val(siswa.kodejurusan);
                          $('#edit_agama').val(siswa.agama);

                          // Logika untuk mengisi radio button Jenis Kelamin
                          if (siswa.jeniskelamin === 'L') {
                              $('#edit_jk_l').prop('checked', true);
                          } else if (siswa.jeniskelamin === 'P') {
                              $('#edit_jk_p').prop('checked', true);
                          }
                      } else {
          ...
          $('#saveSiswaButton').on('click', function() {
              $.ajax({
                  url: 'api_siswa.php',
                  type: 'POST',
                  data: $('#editSiswaForm').serialize() + '&aksi=update',
                  dataType: 'json',
                  success: function(response) {
                      if (response.status === 'success') {
                  ...
                  error: function(jqXHR, textStatus, errorThrown) {
                      ...
                      Swal.fire('Error Kritis!', 'Gagal menyimpan data ke server. Periksa console browser (F12) untuk detail.', 'error');
                  }
              });
          });

          // FUNGSI HAPUS SISWA (Tidak ada perubahan)
          $('#myTable tbody').on('click', '.btn-hapus-siswa', function() {
              var nisn = $(this).data('id');
